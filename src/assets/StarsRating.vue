...
<template>
  <div class="flex items-center">
    <svg
      v-for="n in props.maxStars"
      :key="n"
      xmlns="http://www.w3.org/2000/svg"
      viewBox="0 0 24 24"
      class="size-4"
      :class="n <= Math.round(props.rating) ? 'text-yellow-400 fill-current' : 'text-gray-300 stroke-current fill-none'"
      :fill="n <= Math.round(props.rating) ? 'currentColor' : 'none'"
      :stroke="n <= Math.round(props.rating) ? 'none' : 'currentColor'"
    >
      <path
        v-if="n <= Math.round(props.rating)"
        fill-rule="evenodd"
